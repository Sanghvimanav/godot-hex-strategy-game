            ...
            for group in _groups(state):
                for unit in group.get("units", []):
                    if isinstance(unit, dict):
                        unit_types.add(str(unit.get("unit_type", unit.get("type", UNKNOWN_TOKEN))))
...
class ActionFeaturizer:
    ...
    def encode(self, state: dict[str, Any], action: dict[str, Any]) -> tuple[int, int, list[float]]:
        ...
        unit_type = str(unit.get("unit_type", unit.get("type", UNKNOWN_TOKEN)))
        ...
        return (
            ...
            self.unit_to_id.get(unit_type, self.unit_to_id.get(UNKNOWN_TOKEN, 0)),
            numeric,
        )
