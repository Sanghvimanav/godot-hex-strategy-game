...
def _cmd_plan(args: argparse.Namespace) -> int:
    ...
    if args.results_dir:
        completed = completed_shard_ids(load_shard_results(args.results_dir))
...
def _cmd_missing(args: argparse.Namespace) -> int:
    ...
    if unknown:
        raise ResultContractError(f"results contain shards not present in plan: {sorted(unknown)}")
    payload = {
...
def _cmd_summarize(args: argparse.Namespace) -> int:
    plan = _read_json(args.plan)
    results = load_shard_results(args.results_dir)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    plan.add_argument("--results-dir", help="Existing shard results to treat as completed")
    plan.add_argument("--completed-shard", action="append", default=[])
    plan.add_argument("--output")
    plan.add_argument("--github-output", help="Append matrix and counts to a GitHub Actions output file")
    plan.set_defaults(func=_cmd_plan)

    missing = subparsers.add_parser("missing", help="Compare a plan against downloaded shard results")
