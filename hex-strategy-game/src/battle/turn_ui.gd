...
func _on_replay_pressed() -> void:
	...
	#region agent log
	_agent_debug_log("H1", "turn_ui.gd:_on_replay_pressed", "Replay pressed entry", {
		...
		"has_units_node": _units_node != null,
		"battle_phase": int(_units_node.battle_phase) if _units_node != null else -1
	})
	#endregion
	...
	if not is_planning:
		...
		#region agent log
		_agent_debug_log("H2", "turn_ui.gd:_on_replay_pressed", "Replay blocked by non-planning phase", {
			"has_units_node": _units_node != null,
			"battle_phase": int(_units_node.battle_phase) if _units_node != null else -1
		})
		#endregion
	...
	#region agent log
	_agent_debug_log("H1", "turn_ui.gd:_on_replay_pressed", "Replay request emitted", {
		"turn_to_replay": turn_to_replay,
		"replay_in_progress": _replay_in_progress
	})
	#endregion
