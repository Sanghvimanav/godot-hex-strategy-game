extends RefCounted
class_name PureStateCounterfactualReviewSuite
## Hand-authored, named choices for reviewing strategic behavior examples.
##
## Human review checks that the situation and choices are mechanically coherent.
...
const OPPONENT_MIXTURE_VERSION := "scenario_review_responses_v4"
const CONTINUATION_MIXTURE_VERSION := "scenario_review_continuation_1x1_2x2_v4"

const CONTINUATION_PROFILES := [
	{
		"profile_id": "review_fast_1x1",
		"max_turns": 3,
	...
	{
		"profile_id": "review_balanced_2x2",
		"max_turns": 4,
...
static func _sacrifice_job(rules_version: String) -> Dictionary:
	var state := _state("counterfactual_review_sacrifice", 4, [
		_group("terran", [
...
static func _preservation_job(rules_version: String) -> Dictionary:
	var state := _state("counterfactual_review_preservation", 4, [
		_group("terran", [
...
static func _spreading_job(rules_version: String) -> Dictionary:
	var state := _state("counterfactual_review_spreading", 4, [
		_group("terran", [
			_unit(1, "marine", [0, 0]),
			_unit(2, "marine", [0, 0]),
			_unit(3, "marine", [0, 0]),
...
static func _retreat_job(rules_version: String) -> Dictionary:
	var state := _state("counterfactual_review_retreat", 4, [
		_group("terran", [
...
static func _trapped_unit_job(rules_version: String) -> Dictionary:
	var state := _state("counterfactual_review_trapped_unit", 4, [
		_group("terran", [
	...
	return _job(
		"review-trapped-zergling-last-kill",
		"trapped_zergling",
...
static func _coordinated_commitment_job(rules_version: String) -> Dictionary:
	var state := _state("counterfactual_review_coordinated_commitment", 4, [
		_group("terran", [
			_unit(1, "marine", [0, 0]),
			_unit(2, "marine", [0, 0]),
			_unit(3, "marine", [0, 0]),
...
static func _production_pressure_job(rules_version: String) -> Dictionary:
	var state := _state("counterfactual_review_production_pressure", 4, [
		_group("terran", [
			_unit(1, "marine", [0, 0]),
			_unit(2, "marine", [0, -1]),
			_unit(3, "marine", [-1, 1]),
	...
	return _job(
		"review-attack-zerg-production-early",
		"production_pressure",
...
static func _opponent(sample_id: String, weight: float, actions: Array) -> Dictionary:
	return {
		"sample_id": sample_id,
		"profile_id": "scenario_review_response",
		"weight": weight,
		"actions": actions,
	}
