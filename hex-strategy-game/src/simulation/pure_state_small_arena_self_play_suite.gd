extends RefCounted
class_name PureStateSmallArenaSelfPlaySuite
## Radius-one Marine-vs-Zergling starts for the self-play experiment.
##
## Exact 10-state distribution: 80% 3v3, 10% 2 Marines vs 3 Zerglings,
## 10% 1 Marine vs 2 Zerglings. Positions are seeded, legal, and varied.
...
static func build_state(seed: int, max_turns: int = DEFAULT_MAX_TURNS) -> Dictionary:
    ...
    var state := PureStateSelfPlaySuite.basic_state(
        marines, zerglings, 0, max_turns, "", false
    )
    state["scenario_id"] = "small_selfplay_%s" % str(matchup["id"])
    state["turn_index"] = 0
    ...
    state["small_arena"] = {
        ...
        "distribution": "80pct_3v3_10pct_2v3_10pct_1v2",
    }
    return state
