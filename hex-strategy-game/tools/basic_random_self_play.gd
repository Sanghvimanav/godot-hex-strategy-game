...
func _run() -> void:
	...
	for job_variant in jobs:
		...
		var greedy_settings_a := _settings_for(job, evaluator, checkpoint, "greedy", 0)
		...
		for profile_index in range(PROFILES.size()):
			...
			if profile == "greedy":
				...
				var settings_a := _settings_for(job, evaluator, checkpoint, profile, policy_seed)
	...
	print("[basic-random-self-play] summary evaluator=%s base_games=%d trajectories=%d examples=%d outcomes=%s" % [
		evaluator, jobs.size(), summaries.size(), all_examples.size(), str(outcomes)
	])
	PureStateNeuralEvaluator.shutdown()
	get_tree().quit(0 if ok else 1)


func _settings_for(job: Dictionary, evaluator: String, checkpoint: String, profile: String, seed: int) -> Dictionary:
	...
	if evaluator == "neural":
		settings = GameplayAI.neural_settings(max_actions, own_plans, max_actions, opponent_plans, checkpoint)
	else:
