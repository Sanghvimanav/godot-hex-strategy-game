...
func _run() -> void:
    ...
    for game_index in range(game_count):
        ...
        if valid:
            for turn_variant in rollout.get("history", []):
                ...
                for perspective_group in ["terran", "zerg"]:
                    ...
                    for row_variant in turn.get(prefix_key, []):
                        ...
                        row["scenario_id"] = "small_selfplay_v5_" + matchup
            ...
            var built := PureStateTrainingData.build_examples_from_rollout(
                ...
                game_id,
                {
                    "dataset": "small_arena_self_play_v5_balanced",
        ...
        games.append({
            ...
            "shard_index": shard_index,
            "scenario_seed": scenario_seed,
            "scenario_id": "small_selfplay_v5_" + matchup,
            "matchup": matchup,
            "layout_signature": layout_signature,
    ...
    var manifest := {
        "experiment": "small_arena_self_play_v5_balanced",
        "generation": generation,
        "shard_index": shard_index,
        "games": game_count,
        "hex_radius": 1,
