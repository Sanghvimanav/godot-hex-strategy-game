...
- **TurnExecutionCore** – `find_unit_by_id`, `get_units_at_cell`, `get_unit_def`, `get_damage_cells_for_config` (self, ray, target, area_adjacent), `execute_turn` (move + attack, stunned unit action skip, recording structure, died_ids), `check_win_condition`.
- **ServerTurnExecutor** – `validate_action` (valid move, invalid path, unit not found, dead unit, stunned unit, target out of range), `execute_turn` delegates to core.
