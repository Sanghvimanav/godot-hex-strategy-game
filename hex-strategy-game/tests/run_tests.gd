...
func _ready() -> void:
	...
	var TestPureStateSmallArenaSuite = load("res://tests/test_pure_state_small_arena_suite.gd") as GDScript
	...
	var TestPureStateCandidateOracleRecall = load("res://tests/test_pure_state_candidate_oracle_recall.gd") as GDScript
	...
	if not TestPureStateSelfPlaySuite.run_all(self):
		...
		_pass_count += 1
	if not TestPureStateSmallArenaSuite.run_all(self):
		_fail_count += 1
	else:
		_pass_count += 1
	if not TestPureStateSelfPlayDiversity.run_all(self):
		_fail_count += 1
	else:
		_pass_count += 1
	...
	if not TestPureStateCounterfactualAnswerKey.run_all(self):
		...
		_pass_count += 1
	if not TestPureStateCandidateOracleRecall.run_all(self):
		_fail_count += 1
	else:
		_pass_count += 1
