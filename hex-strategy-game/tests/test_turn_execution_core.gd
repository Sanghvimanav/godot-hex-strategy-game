...
static func _test_execute_turn_resupply_after_scout_attack_same_turn(tests: Node) -> bool:
	tests._log("test_turn_execution_core: non-origin base resupplies mage after flame attack same turn")
	...
	if mage.is_empty():
		tests._fail("mage should exist after turn")
		return false
	var mage_energy: int = int(mage.unit.get("energy", -1))
	if mage_energy != 3:
		tests._fail("non-origin resupply target should be absolute: mage should have 3 energy (attack 4->2, resupply +1 -> 3), got %d" % mage_energy)
		return false
	tests._pass("non-origin base resupplies mage after flame attack same turn")
	return true
